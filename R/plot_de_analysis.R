...
#' @param celltype_counts vector with the counts of cells after QC in each cell type
...
plot_de_analysis <- function(pb_dat,y,celltype_DEGs_dt,celltype_all_genes_dt,
    ...
    data.table::setnames(top_degs_pseudobulk_exp,c("celltype","name","group_sample",
                                        "expression"))
    ...
    top_degs_pseudobulk_exp_plot <-
        ...
        ggplot2::stat_summary(fun.data = "mean_cl_normal",
                     ...
                     geom = "crossbar",
                     show.legend = T)+
        ggplot2::scale_shape_manual("", values=c("mean"="x"))+
        ggplot2::labs(y= "Sum pseudobulk expression counts (unnormalised)",
                x = "Phenotype",
    ...
    counts_celltypes_dt <- data.table::data.table(celltype=names(counts_celltypes),
    ...
    cell_counts_plot <-
        ...
        ggplot2::geom_text(ggplot2::aes(x = celltype, y = counts, label = counts),
                  vjust=-0.25,size=3) +
        cowplot::theme_cowplot()+
    ...
    deg_per_cell_type_plot <-
        ...
        ggplot2::geom_text(ggplot2::aes(x = celltype, y = N, label = N),
                  vjust=-0.25,size=3) +
        cowplot::theme_cowplot()+
    ...
    deg_prop_plot <-
        ...
        ggplot2::labs(y= "Proportion of DEGs identified", x = "Cell Type",
                fill="Cell Type") +
        ggplot2::geom_text(ggplot2::aes(x = factor(celltype), y=prop,
    ...
    # check median log2 fold change for both up and down regulated genes; this will give a sense of directional effect size
    ...
    degs_boxplot_plots<-
        ...
        ggplot2::facet_wrap(~factor(celltype),scales="free")+
        ggplot2::ggtitle("Cell type Log2 Fold Change of DEGs",
