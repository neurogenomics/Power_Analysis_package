...
#' @return a list of size Nperms, with each item being a subset of the input data containing the specified number of individuals

sample_individuals <- function(SCE,
                               Nsamples, 
                               sampleID="Donor.ID", 
                               sexID="sex",
                               Nperms=20){
    ...
    for(j in 1:Nperms){
        ...
        while(numMales==0|numFemales==0){
            ...
            for(sample in subSamples){
                if(unique(coldata[coldata[,sampleID]==sample,][[sexID]]) == unique(SCE[[sexID]])[[2]]){
                    numMales <- numMales + 1
                }else{
        ...
        newData[[j]] <- SCE[, SCE[[sampleID]]%in%subSamples]
    }
