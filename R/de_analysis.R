...
#' @param formula the validated design formula of class type `formula`. Equation used to fit the model- data for the generalised linear model e.g. ~ sex + pmi + disease.
#' @param y_name the column name in the SCE object for the return variable e.g. "diagnosis" - Case or disease. y can be discrete (logisitc regression) or continuous (linear regression)
#' @param y_contin is the variable being modelled continuous e.g. if case/control then TRUE if level of Tau in AD study then FALSE
...
#' @param adj_pval the adjusted p-value cut-off for the differential expression analysis, 0-1 range
        ...
        if(!y_contin){# adjust levels based on user input if not contin
            ...
            if(length(old_levels)>2){
                if(!is.null(coef)){# if user inputted value for case samples
                    ...
                    if(is.null(control)){
                        message("WARNING: No control passed so may not be compared against desired variable")
...
>>>>>>> 6815eb61a3a99ac648943b608f4947aad63320da
