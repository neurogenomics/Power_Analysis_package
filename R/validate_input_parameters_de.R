...
#' @param celltype_ID Column name in the SCE object for the cell type variable 
...
validate_input_parameters_de<-function(SCE, design, pseudobulk_ID, celltype_ID, 
    ...
    if(!is.character(celltype_ID))
        stop("Please input a character for celltype_ID indicating the column holding the cell type information")
    ...
    if(!inherits(design,"formula"))
        stop("Please input a formula for the design variable specifying the comparison. See examples.")
    ...
    if(!grepl( "~", design_txt, fixed = TRUE))
        stop("Please input a correctly formated formula for the design variable containing a `~`. See examples.")
